...
class DataProcessor(Config):
    def __init__(self):
        ...
        if self.params['dataset'] == 'msr':
            self.split_test_train = self.split_msr
        elif self.params['dataset'] == 'ntu':
            self.split_test_train = self.split_ntu
        else:
    ...
    def split_msr(self):
        ...
        for index, filename in enumerate(glob.iglob(self.params['rgb_raw_path'], recursive=True)):
    ...
    def data_aug(skel_data, rgb_data, frame_info):
        """Rotate the image.
    ...
    def process_msr_data(self):
        ...
        for index, filename in tqdm(enumerate(glob.iglob(self.params['skeleton_raw_path'],
            ...
            rgb_data = self.normalize_rgb(rgb_data)
...
def main():
    ...
    pre_process = DataProcessor()
    # partiion, labels = pre_process.split_test_train()

    # print('label: ', pre_process.labels)
    # print('partition: ', pre_process.partition)

    # process everything together
    if not os.path.exists(pre_process.params['skeleton_data']):
        os.makedirs(pre_process.params['skeleton_data'])
    if not os.path.exists(pre_process.params['rgb_data']):
        os.makedirs(pre_process.params['rgb_data'])
    pre_process.process_msr_data()
